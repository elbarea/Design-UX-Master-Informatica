...
var bValidar= true;
var bARQIncluirFicheroMensajes= true;
...
//Propiedades validas para tiempos
var prHourMinuteOnly      =	prBase + 8;
...
{
//	bARQIncluirFicheroMensajes = true;
   if ((bARQIncluirFicheroMensajes) || self['validacionGeneral'] || (self['oArrayValidaciones'] != undefined))
...
function validaDatos(oFormulario)
{
	...
	//Añadimos las validaciones para la subida de ficheros
	enc=false;
	var arrayUploads=$('input[type=file]');
	var oArrayValidacionesAux=oArrayValidaciones.slice();
	for(i=0;i<oArrayValidaciones.length && !enc;i++)
	{
		if((oArrayValidacionesAux[i].validacion).indexOf("validarEnvio")!=-1)
		{
			oArrayValidacionesAux.splice(i,oArrayValidaciones.length);
			enc=true;
		}
	}
	oArrayValidaciones=oArrayValidacionesAux.slice();
	
	for(i=0;i<arrayUploads.length;i++)
	{
		//(id, nombre, longmin, obligatorio, tipodato, validacion, cualificador, idEV, concepto, idMensaje, dominio, botones, enteros, decimales, negativo)
		oAux = new oValida(arrayUploads[i].id,arrayUploads[i].id,"0",0,"","validarEnvio('"+arrayUploads[i].id+"')","","",arrayUploads[i].id,"","","","0","0","");
		oArrayValidaciones.push(oAux);
	}
	...
	{				
		...
		if (oValidacion.id.length > 0) {
			...
			oElemen = document.getElementsByName(oValidacion.nombre);
		}
		
		if (oElemen!=null && oElemen.type == undefined && oElemen[0]!=null && (oElemen[0].type == "text" || oElemen[0].type == "password" || oElemen[0].type == "textarea" || oElemen[0].type == "file" || oElemen[0].type.substring(0,6) == "select" || oElemen[0].type == "radio" || oElemen[0].type == "checkbox")){
			oElemen = oElemen[0];
		}
...
function validaDato(oElemen, oValidacion)
{
	...
	var sNegativo    = oValidacion.negativo;
		...
		if (sTipoDato) {
			...
			if (bTipoDatoArquitectura) {
				...
				if (oParam) {
					...
					if (sTipoDato == 'dtRadioButton' || sTipoDato == 'dtDecimal') {
						oMsgError = oARQDFuncValidacion[getFnIndex(itipoDato)](oValidacion);
					}else if (sTipoDato == 'dtMonedaEuro'){
						oMsgError = oARQDFuncValidacion[getFnIndex(itipoDato)](sValor, oValidacion.concepto, sNegativo);												
					}
					else {
				...
				if (idex != -1) {
					...
					if (oRetFuncPart.codigo.length > 0) {
						return (sIdError == null || sIdError == "") ? oRetFuncPart : (new ParqMensaje(sIdError,null,'ARQ'));
					}
				} else {
	...
	if (sValidacion != undefined && sValidacion != '')
	{
		...
		{ 
			sValidacion= sValidacion + (sValidacion.indexOf("()")==-1 && sValidacion.indexOf("(")==-1?"()":"") ;
		...
		else
		{
			if(ProsaUpload)
			{
				oRetValPart=ProsaUpload.validarEnvio(oElemen.id);
				var typRet= typeof(oRetValPart);
				if (typRet.toUpperCase() == "STRING")
				{
					return new ParqMensaje(oRetValPart, null);
...
function oValida(id, nombre, longmin, obligatorio, tipodato, validacion, cualificador, idEV, concepto, idMensaje, dominio, botones, enteros, decimales, negativo)
{
	this.id= id;
	this.nombre = nombre;
...
function isDtInteger(buf,parNombre,sCualif)
{
	...
	{
		return new ParqMsgFormatoIncorrecto(parNombre, "numérico entero", buf);
	}
}
...
//*************************************************************************
//* isDtMonedaEuro: Funcion encargada de comprobar	si la cadena pasada   *
//* como parametro se corresponde con una cantidad monetaria expresada en *
//* Euros.																  *
//* @param buf: cadena de caracteres a validar   						  *
//* @param parNombre: nombre con el que referirse al campo en un supuesto *
//* mensaje   						  									  *
//* @param permNegativo: permite valores negativos si valiese 1			  *
//* @return devuelve una cadena vacía si se cumple la validación, o bien  *
//*		un texto de error si éste se produce               				  *
//*************************************************************************	
function isDtMonedaEuro(buf,parNombre, permNegativo)
{
	if (isDtFormatted(buf,parNombre,(permNegativo=='1')?"^-?[0-9]{1,11}([,|.][0-9][0-9]?)?$":"^[0-9]{1,11}([,|.][0-9][0-9]?)?$").codigo=="")
		return new ParqMensaje("",null);
	else
	{
...
//*************************************************************************
//* isDtDecimal: Funcion encargada de comprobar	si la cadena pasada como  *  
//*	parametro sea un número que contenga un máximo de n caracteres en la  *
//*	parte entera, y, si se infoma, un máximo de m caracteres en la parte  *
//*	 decimal, si no, sin decimales. 									  *
//* @param oValidacion: objeto oValida que contiene la información 		  *
//* necesaria(*enteros y decimales) para realizar la validacion			  *
//* @return devuelve una cadena vacía si se cumple la validación, o bien  *
//*		un texto de error si éste se produce               				  *
//*************************************************************************	
function isDtDecimal(oValidacion)
{
	var sInputName = oValidacion.nombre;
	var oInput = document.getElementsByName(sInputName)[0];
	if (oValidacion.decimales == 0) {
		var sExpresion = "^[-]?[0-9]{1," + oValidacion.enteros + "}$";
	} else {
		var sExpresion = "^[-]?[0-9]{1," + oValidacion.enteros + "}([,|.][0-9]{1," + oValidacion.decimales + "}?)?$";
	}

	if (isDtFormatted(oInput.value, oValidacion.concepto, sExpresion).codigo == "") {
		return new ParqMensaje();
	} else {
		return new ParqMsgFormatoIncorrecto(oValidacion.concepto, "Numérico con " + oValidacion.enteros + " dígitos enteros y " + oValidacion.decimales + " dígitos decimales", oInput.value);
	}
}
oARQDFuncValidacion[getFnIndex(dtDecimal)]=isDtDecimal;
...
//*		un texto de error si éste se produce               				  *
//*************************************************************************	
function isDtTime(buf, parNombre, sCualif)
{
	var bRet= false;
	if(buf	&& sSepHora && trim(buf).length)
	{
		...
		if	(sCualif && sCualif == prHourMinuteOnly) {
			var sExpresion = "^([01]|2(?=[0-3]))[0-9]"+ sSepHora + "[0-5][0-9]$";
		} else {
			var sExpresion = "^([01]|2(?=[0-3]))[0-9]("+ sSepHora + "[0-5][0-9]){2}$";
		}
		bRet = (isDtFormatted(buf,parNombre,sExpresion).codigo=="");
	}
...
function isDtDate(buf, parNombre)
{
	...
	if (buf != null && trim(buf).length)
	{
		...
		sPat2Apply     +=  "(17|18|19|20|21)[0-9]{2}$";
...
{
	if (isDtFormatted(buf,parNombre,"^(17|18|19|20|21)[0-9][0-9]$").codigo=="")
		return new ParqMensaje("",null);
	else
	{
...
function isDtDateTime(buf,parNombre)
{
	...
	if (buf)
	{
		...
		sPat2Apply	+= sSepFec + "(17|18|19|20|21)[0-9]{2}";	 											//Anio
