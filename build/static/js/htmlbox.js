...
$.fn.htmlbox=function(options){
    ...
    var colors = (typeof document.htmlbox_colors === 'function')?document.htmlbox_colors():['servicios','silver','silver','white','white','yellow','yellow','orange','orange','red','red','green','green','blue','blue','brown','brown','black','black'];
	...
	var d={
		...
		idir:"/CPMSCitaPrevia/PROS/js/v3/images/",// HtmlBox Image Directory, This is needed for the images to work
		...
		for(var k=0;k<d.toolbars.length;k++){
			...
			for(var i=0;i<(toolbar.length);i++){
	            ...
	            // END: Custom button
				    ...
				    h += "<td valign='middle' align='center'><select id='"+d.id+"_syntax' onchange='global_hb[\""+d.id+"\"].cmd(\"syntax\",this.options[this.selectedIndex].value);this.options[0].selected=\"true\";' style='font-size:12px;'><option value='' selected>-LENGUAJE-</option>";
				...
				// Commands
	            var cmds = {"about":"Acerca","bold":"Negrita","center":"Centrar","code":"Ver Código","copy":"Copiar","cut":"Cortar","hr":"Insertar Línea","link":"Insertar Enlace","image":"Insertar Imagen","indent":"Indentar","italic":"Italica","justify":"Justificar","left":"Izquierda","ol":"Lista Ordenada","outdent":"Outdent","paragraph":"Insertar Párrafo","paste":"Pegar","quote":"Quote","redo":"Rehacer","removeformat":"Eliminar Formatos","right":"Derecha","strike":"Tachar","striptags":"Quitar Tags","sub":"Subscript","sup":"Superscript","ul":"Lista","underline":"Subrayar","undo":"Deshacer","unlink":"Quitar enlace"};
			...
			if(d.muestraExpandir==="1"){
				h += "<td valign='middle' align='center'>";
				h += "<input type=\"image\" title=\"Expandir\" onclick=\"return expandeEnriq('"+d.id+"');\" src=\"PROS/img/comun/ParqBoton.gif\" name=\"botonImag\" id=\"botonImag\"/>";
				h += "</td>";
			}
			h += "</table></td></tr>";
		}
	...
	this._init = function(is_init){
		...
		var h=$(this).css("height");$(this).wrap("<table id='"+d.id+"_wrap' width='"+w+"' style='height:"+h+";border:2px solid #E9EAEF;' cellspacing='0' cellpadding='0'><tr><td id='"+d.id+"_container'></td></tr></table>");
		...
		if(d.skin==="servicios"){
			hb_border = "1px solid #DDDDDD";
			hb_background = "#d3f3d5";
			tb_border = "1px solid #DDDDDD";
		}
		if(d.skin==="tareas"){
			hb_border = "1px solid #DDDDDD";
			hb_background = "#D2E4F3";
			tb_border = "1px solid #DDDDDD";
		}
		if(d.skin==="altocontraste"){
			hb_border = "1px solid #00FF00";
			hb_background = "#00FF00";
			tb_border = "1px solid #00FF00";
		}
		if(d.skin==="comun" || d.skin==="dispMovil" || d.skin==="internet" || d.skin==="internetAnonimo" || d.skin==="internetPortal" || d.skin==="intranetDS"){
			hb_border = "1px solid #1F2B37";
			hb_background = "#DCE1E5";
			tb_border = "1px solid #1F2B37";
		}
	...
	this.cmd = function(cmd,arg1){
	   ...
	   if(cmd==="code"){
		   ...
		   d.idoc.execCommand("createlink", false, prompt("Pegar enlace URL aquí::"));
	   }else if(cmd==="image"){
		   d.idoc.execCommand("insertimage", false, prompt("Pegar URL del enlace a la imagen aquí:"));
	       ...
	       if($.browser.msie === false){
		       alert("Sólo en Internet Explorer.\nUsar CTRL+X para cortar texto!");
		   }else{
	           d.idoc.execCommand('Cut');
	       }
	   }else if(cmd==="copy"){
	       if($.browser.msie === false){
		       alert("Sólo en Internet Explorer.\nUse CTRL+C para copiar texto!");
		   }else{
	           d.idoc.execCommand('Copy');
	       }
	   }else if(cmd==="paste"){
	       if($.browser.msie === false){
		       alert("Sólo en Internet Explorer.\nUsar CTRL+V para pegar el texto!");
		   }else{
